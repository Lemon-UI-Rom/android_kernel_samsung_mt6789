...
EXPORT_SYMBOL(is_hard_reset_occurred);
...
static int hard_reset_hook_init(void)
{
	size_t i;
...
extern int sec_kn_init(void);
extern int sec_crash_key_init(void);
extern int sec_debug_init_crash_key_user(void);
#endif

extern void register_mrdump_reset_delay(void (*func)(void));

static int __init sec_reset_init(void)
{
#if IS_ENABLED(CONFIG_SEC_KEY_NOTIFIER)
	sec_kn_init();
	sec_crash_key_init();
	sec_debug_init_crash_key_user();
...
module_init(sec_reset_init);

MODULE_DESCRIPTION("Samsung Reset driver");
